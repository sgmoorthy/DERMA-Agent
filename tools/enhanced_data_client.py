...
class EnhancedGDCClient:
    ...
    def fetch_clinical_data(
        self, project_id: str, limit: int = 500, local_mirror_dir: Optional[str] = None
    ) -> pd.DataFrame:
        ...
        if local_mirror_dir:
                            ...
                            if isinstance(data, dict) and "data" in data:
                                ...
                                if hits:
                                    ...
                                    for case in hits:
                                        ...
                                        diagnoses = (
                                            case.get("diagnoses", [{}])[0]
                                            if case.get("diagnoses")
                                            else {}
                                        )
                                        exposures = (
                                            case.get("exposures", [{}])[0]
                                            if case.get("exposures")
                                            else {}
                                        )
                                        record = {
                                            ...
                                            "age_at_diagnosis": diagnoses.get(
                                                "age_at_diagnosis"
                                            ),
                                            "primary_diagnosis": diagnoses.get(
                                                "primary_diagnosis"
                                            ),
                                            "tumor_stage": diagnoses.get("tumor_stage"),
                                            "morphology": diagnoses.get("morphology"),
                                            "tissue_or_organ_of_origin": diagnoses.get(
                                                "tissue_or_organ_of_origin"
                                            ),
                                            "site_of_resection_or_biopsy": diagnoses.get(
                                                "site_of_resection_or_biopsy"
                                            ),
                                            "days_to_recurrence": diagnoses.get(
                                                "days_to_recurrence"
                                            ),
                                            "days_to_last_follow_up": diagnoses.get(
                                                "days_to_last_follow_up"
                                            ),
                                            "progression_or_recurrence": diagnoses.get(
                                                "progression_or_recurrence"
                                            ),
                                            "prior_malignancy": diagnoses.get(
                                                "prior_malignancy"
                                            ),
                                            "synchronous_malignancy": diagnoses.get(
                                                "synchronous_malignancy"
                                            ),
                                            "lymph_nodes_examined_count": diagnoses.get(
                                                "lymph_nodes_examined_count"
                                            ),
                                            "lymph_node_involved_site": diagnoses.get(
                                                "lymph_node_involved_site"
                                            ),
                                            "cigarettes_per_day": exposures.get(
                                                "cigarettes_per_day"
                                            ),
                                            "years_smoked": exposures.get(
                                                "years_smoked"
                                            ),
                                            "alcohol_history": exposures.get(
                                                "alcohol_history"
                                            ),
                                            "alcohol_intensity": exposures.get(
                                                "alcohol_intensity"
                                            ),
                                        }
            ...
            print(
                f"Warning: local_mirror_dir specified but no clinical file found for {project_id} in {local_mirror_dir}. Falling back to GDC API/Cache."
            )
        ...
        try:
            ...
            for case in cases:
                demographic = case.get("demographic", {}) or {}
                diagnoses = (
                    case.get("diagnoses", [{}])[0] if case.get("diagnoses") else {}
                )
                exposures = (
                    case.get("exposures", [{}])[0] if case.get("exposures") else {}
                )
    ...
    def fetch_mutation_data(
        self, project_id: str, gene_list: List[str] = None, limit: int = 1000
    ) -> pd.DataFrame:
        ...
        params = {"project_id": project_id, "size": limit}
    ...
    def fetch_gene_expression(
        self, project_id: str, gene_ids: List[str], limit: int = 100
    ) -> pd.DataFrame:
        ...
        return pd.DataFrame(
            {
                ...
                "vital_status": np.random.choice(
                    ["Alive", "Dead"], n_samples, p=[0.6, 0.4]
                ),
...
class MultiSourceDataClient:
    ...
    def fetch_gdc_release_version(self) -> str:
        ...
        try:
            ...
            if r.status_code == 200:
                self.gdc_release_version = r.json().get(
                    "data_release", "GDC Active Release"
                )
            else:
    ...
    def fetch_comprehensive_clinical_data(
        self,
        project_id: str,
        ...
        results["clinical"] = self.gdc_client.fetch_clinical_data(
            project_id, local_mirror_dir=local_mirror_dir
        )
        ...
        if include_mutations:
            ...
            results["mutations"] = self.gdc_client.fetch_mutation_data(
                project_id, key_genes
            )

        return results

    def fetch_multiple_cohorts(
        self,
        ...
        if parallel and len(project_ids) > 1:
            with ThreadPoolExecutor(max_workers=3) as executor:
                future_to_project = {
                    executor.submit(
                        self.gdc_client.fetch_clinical_data, pid, 500, local_mirror_dir
                    ): pid
            ...
            for project_id in tqdm(project_ids, desc="Fetching cohorts"):
                results[project_id] = self.gdc_client.fetch_clinical_data(
                    project_id, local_mirror_dir=local_mirror_dir
                )

        return results

    def get_survival_analysis_ready_data(
        self,
        project_id: str,
        ...
        df = self.gdc_client.fetch_clinical_data(
            project_id, local_mirror_dir=local_mirror_dir
        )
        ...
        df["stage_group"] = df["tumor_stage"].apply(
            lambda x: (
                "Early"
                if x in ["stage i", "stage ia", "stage ib"]
                else (
                    "Advanced"
                    if x in ["stage iii", "stage iiia", "stage iiib", "stage iv"]
                    else "Intermediate"
                )
                if pd.notna(x)
                else "Unknown"
    ...
    def merge_pathology_features(
        self, df_clinical: pd.DataFrame, pathology_csv_path: str
    ) -> pd.DataFrame:
        """Merge histopathology features into the clinical dataset by submitter_id / case_id."""
        if not os.path.exists(pathology_csv_path):
            print(
                f"Warning: Pathology feature CSV '{pathology_csv_path}' not found. Returning clinical data unchanged."
            )
            return df_clinical
        ...
        try:
            ...
            if not common_id:
                print(
                    "Warning: No matching ID column ('submitter_id' or 'case_id') found in pathology features. Cannot merge."
                )
                return df_clinical
            ...
            print(
                f"Successfully merged {len(df_pathology)} pathology records. New columns added: {list(rename_dict.values())}"
            )
            return df_merged
        except Exception as e:
