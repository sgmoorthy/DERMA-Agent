...
from derma_agent.secrets import get_secret, has_secret
...
try:
    ...
    from tools.enhanced_clinical_stats import (
        EnhancedStatsEngine,
...
def load_latest_discovery():
    ...
    if discovery_files:
        ...
        if isinstance(payload, dict) and "ledger" in payload:
            return payload.get("ledger", []), latest_file
        if isinstance(payload, list):
            return payload, latest_file
    return [], None
...
def render_knowledge_fabric_tab():
    ...
    if not ENHANCED_MODE:
        st.warning(
            "Knowledge Fabric not available. Please install enhanced dependencies."
        )
        return
    ...
    try:
        ...
        node_df = pd.DataFrame(
            list(stats["node_types"].items()), columns=["Node Type", "Count"]
        )
        fig = px.pie(
            node_df,
            values="Count",
        ...
        query_type = st.selectbox(
            "Query Type",
            ["Drugs for Cancer", "Genes for Cancer", "Pathway Members", "Drug Targets"],
        )

        if query_type == "Drugs for Cancer":
            cancer = st.selectbox(
                "Select Cancer",
                ["Melanoma", "Breast_Carcinoma", "Lung_Adenocarcinoma", "Glioblastoma"],
            )
            if st.button("Query"):
                drugs = []
                            ...
                            drugs.append(
                                {
                                    ...
                                    "Target": drug_node.properties.get(
                                        "target", "Unknown"
                                    ),
                                    **edge.properties,
                                }
            ...
            drug = st.selectbox(
                "Select Drug",
                ["Pembrolizumab", "Trastuzumab", "Dabrafenib", "Temozolomide"],
            )
            if st.button("Query"):
                targets = []
                for edge in kg.edges:
...
def render_discovery_tab():
    ...
    with st.expander("Launch Discovery Run", expanded=(not ledger)):
        ...
        if recipe == "Prognostic Biomarkers in Skin Melanoma (SKCM)":
            ...
            default_cohorts = ["Skin Cancer"]
            st.info(
                "Recipe selected: Evaluates prognostic impact of gene mutations, stage, and pathology in Melanoma."
            )
            ...
            st.info(
                "Recipe selected: Assesses mutation patterns, clinical features, and pathway interactions in Breast Cancer."
            )
            ...
            st.info(
                "Recipe selected: Scans multiple cohorts to find common gene-survival linkages across cancer types."
            )
        ...
        with col1:
            ...
            sig_thresh = st.slider(
                "Significance Threshold (p-value)", 0.01, 0.1, default_p, step=0.01
            )
        with col2:
        ...
        if st.button("🚀 Execute Discovery Run"):
            if not selected_cohorts:
                ...
                with st.spinner(
                    "Executing discovery engine. Running stats, ast-checking, and generating reports..."
                ):
                    try:
                        # Translate cohort names to project IDs
                        project_ids = [
                            EXPANDED_CANCER_PROJECTS[name] for name in selected_cohorts
                        ]
                        ...
                        cohort_names_mapped = [
                            name.replace(" Cancer", "").replace(" Carcinoma", "")
    ...
    if not ledger:
        st.info(
            "No discovery results found. Run a discovery using the generator above!"
        )
        return
...
- **FDR Method:** {report.get("fdr_method", "benjamini-hochberg") if report else "benjamini-hochberg"}
- **FDR-Significant Findings (q <= {report.get("fdr_alpha", report.get("config", {}).get("significance_threshold", 0.05)) if report else 0.05}):** {report.get("significant_findings", 0) if report else sum(1 for e in ledger if e.get("significant"))}
- **Raw Significant Findings (p <= {report.get("config", {}).get("significance_threshold", 0.05) if report else 0.05}):** {report.get("raw_significant_findings", sum(1 for e in ledger if e.get("raw_significant")) if ledger else 0) if report else sum(1 for e in ledger if e.get("raw_significant"))}
...
| Project ID | Hypothesis | Raw P-Value | FDR Q-Value | Hazard Ratio | FDR Significant |
|------------|------------|-------------|-------------|--------------|-----------------|
"""
        for entry in ledger:
            p_val_str = (
                f"{entry.get('p_value'):.4f}"
                if entry.get("p_value") is not None
                else "N/A"
            )
            q_val_str = (
                f"{entry.get('adjusted_p_value'):.4f}"
                if entry.get("adjusted_p_value") is not None
                else "N/A"
            )
            hr_str = (
                f"{entry.get('hazard_ratio'):.2f}"
                if entry.get("hazard_ratio") is not None
                else "N/A"
            )
            sig_str = "Yes" if entry.get("significant") else "No"
            report_md += f"| {entry.get('project_id')} | {entry.get('hypothesis')} | {p_val_str} | {q_val_str} | {hr_str} | {sig_str} |\n"

        report_md += "\n## Individual Hypotheses & Conclusions\n"
        for idx, entry in enumerate(ledger):
            report_md += f"### {idx + 1}. {entry.get('hypothesis')}\n"
            report_md += f"- **Cohort:** {entry.get('project_id')}\n"
            report_md += f"- **Raw P-Value:** {entry.get('p_value')}\n"
            if entry.get("adjusted_p_value") is not None:
                report_md += f"- **FDR Q-Value:** {entry.get('adjusted_p_value'):.4f}\n"
    ...
    with st.expander("Generate Kaplan-Meier Curves from Cohort Data", expanded=False):
        cohort_list = list(
            set(
                [entry.get("project_id") for entry in ledger if entry.get("project_id")]
            )
        )
        if not cohort_list:
            ...
            with st.spinner("Fetching and prepping data..."):
                try:
                    ...
                    if not df_clin.empty:
                        ...
                        pathology_csv = (
                            f"data/pathology_{sel_cohort.lower()}_features.csv"
                        )
                        if os.path.exists(pathology_csv):
                            df_clin = data_client.merge_pathology_features(
                                df_clin, pathology_csv
                            )
                        ...
                        cols_to_plot = [
                            ...
                            if df_clin[c].nunique() > 1 and df_clin[c].nunique() < 10
                        ]
                        cont_cols = [
                            c
                            for c in df_clin.select_dtypes(include=[np.number]).columns
                            if c
                            not in [
                                "time",
                                "event",
        ...
        if "plot_df" in st.session_state and "cols_to_plot" in st.session_state:
            ...
            plot_col = st.selectbox(
                ...
                cols + [f"{c} (Split by Median)" for c in conts],
            )
            ...
            if st.button("Plot Kaplan-Meier Curve"):
                ...
                if "Split by Median" in plot_col:
                    ...
                    df_plot["temp_group"] = df_plot[base_col].apply(
                        lambda x: (
                            f"High (>= {median_val:.2f})"
                            if x >= median_val
                            else f"Low (< {median_val:.2f})"
                        )
                    )
                ...
                ax.set_title(
                    f"Kaplan-Meier Survival Curves by {plot_col} (Cohort: {sel_cohort})"
                )
    ...
    for entry in ledger:
        ...
        with st.container():
            ...
            with col2:
                ...
                if entry.get("adjusted_p_value") is not None:
                    st.markdown(f"**q = {entry['adjusted_p_value']:.4f}**")
                if is_sig:
                    st.markdown("🎯 **FDR SIGNIFICANT**")
            ...
            with st.expander("Details"):
                ...
                if entry.get("adjusted_p_value") is not None:
                    st.write(f"**FDR Q-Value:** {entry['adjusted_p_value']:.4f}")
                if entry.get("raw_significant") is not None:
                    st.write(
                        f"**Raw-significant before FDR:** {'Yes' if entry['raw_significant'] else 'No'}"
                    )
                if entry.get("hazard_ratio"):
...
def render_pathology_tab():
    ...
    if st.button("🔬 Generate & Analyze"):
        with st.spinner("Analyzing tissue..."):
            ...
            with col2:
                st.image(
                    mask.astype(float), caption="Nuclei Mask", use_column_width=True
                )
            ...
            if features.texture_features:
                ...
                texture_df = pd.DataFrame(
                    list(features.texture_features.items()),
...
def render_settings_tab():
    ...
    config["significance_threshold"] = st.slider(
        "Significance Threshold (p-value)", 0.01, 0.1, 0.05
    )
    ...
    openai_configured = has_secret("OPENAI_API_KEY")
    google_configured = has_secret("GOOGLE_API_KEY")

    st.write(
        f"OpenAI API Key: {'✅ Configured' if openai_configured else '❌ Not set'}"
    )
    st.write(
        f"Google API Key: {'✅ Configured' if google_configured else '❌ Not set'}"
    )

    if not openai_configured and not google_configured:
        st.info(
            "Set `OPENAI_API_KEY` and/or `GOOGLE_API_KEY` via environment variables or Streamlit secrets for full hosted functionality."
        )
    else:
        st.caption(
            "Secrets can be provided either as environment variables or through Streamlit secrets settings."
        )
...
def main():
    ...
    st.markdown("---")
    st.markdown(
        "<center>DERMA-Agent | Powered by LangGraph, OpenAI, and TCGA</center>",
        unsafe_allow_html=True,
    )
