...
def get_llm():
    ...
    if not api_key:
        return None
    try:
        return ChatOpenAI(model="gpt-4-turbo", temperature=0.2, api_key=api_key)
...
def formulate_hypothesis_node(state: AgentState):
    ...
    if not llm:
        return {
            "current_hypothesis": "Mock Hypothesis: High lymphocyte density correlates with survival."
        }
...
def write_code_node(state: AgentState):
    ...
    if not llm:
        # Mock code for MVP
        mock_code = (
            "print('Validating mock hypothesis...')\n"
...
def build_graph():
    ...
    workflow.add_conditional_edges(
        "execute_code",
        route_evaluation,
        {"write_code": "write_code", "conclude": "conclude"},
    )
