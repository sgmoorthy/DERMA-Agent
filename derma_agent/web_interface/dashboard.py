...
from derma_core.agents.discovery_engine import DiscoveryConfig, FastDiscoveryEngine
...
def _benjamini_hochberg(
    ...
    """Session-wide Benjamini-Hochberg correction for dashboard runs."""
    if not p_values:
        return [], []
...
def _refresh_discovery_run_fdr(alpha: float = 0.05) -> None:
    """Update run status and adjusted p-values across the whole dashboard session."""
    runs = st.session_state.discovery_runs
    tested_runs = [
        (idx, run) for idx, run in enumerate(runs) if run.get("p_value") is not None
    ]
    if not tested_runs:
        return

    adjusted, significant = _benjamini_hochberg(
        [run["p_value"] for _, run in tested_runs],
        alpha=alpha,
    )

    for run in runs:
        raw_p = run.get("p_value")
        run["raw_significant"] = bool(raw_p is not None and raw_p < alpha)
        run["adjusted_p_value"] = None
        run["status"] = "rejected"
        run["fdr_alpha"] = alpha
        run["fdr_method"] = "benjamini-hochberg"

    for (idx, run), q_value, is_significant in zip(tested_runs, adjusted, significant):
        run["adjusted_p_value"] = float(q_value)
        run["status"] = "confirmed" if is_significant else "rejected"
        run["summary"] = (
            f"BRAF mutation {'is' if is_significant else 'is not'} prognostic in {run['cohort']} "
            f"(p={run['p_value']:.4f}, q={q_value:.4f}; medians: "
            f"{run['mut_median']:.0f}d vs {run['wt_median']:.0f}d)"
        )
...
def plot_kaplan_meier(df: pd.DataFrame) -> io.BytesIO:
    ...
    for label, mask in [("BRAF Mutated", mut_mask), ("BRAF WT", wt_mask)]:
        ...
        kmf.plot_survival_function(
            ax=ax, ci_show=True, color=palette[label], linewidth=2.5, alpha=0.9
        )
    ...
    ax.set_xlabel("Days", color="#aaaaaa")
    ax.set_ylabel("Survival Probability", color="#aaaaaa")
...
def plot_tissue_radar(titan_features: dict) -> io.BytesIO:
    ...
    pattern_score = {
        ...
        "sheet-like": 0.45,
        "mixed": 0.70,
    }.get(titan_features.get("primary_pattern", "mixed"), 0.5)
    ...
    ax.set_title(
        "TITAN Tissue Features", color="#f0f0f0", fontsize=11, fontweight="bold", pad=14
    )
    fig.tight_layout()
    return _buf(fig)
...
def render_pyvis_kg(fabric: KnowledgeFabric, height: int = 420) -> str:
    ...
    def _node_color(n: str) -> str:
        if n.startswith("TCGA"):
            return "#e63946"
        if n in ("BRAF", "TP53", "EGFR", "PTEN", "CDH1", "PIK3CA"):
            return "#4cc9f0"
        if "Pathway" in n:
            return "#f4a261"
        if n in (
...
def plot_apollo_embedding(embedding: np.ndarray, slide_id: str) -> io.BytesIO:
    ...
    ax1.set_title(
        "APOLLO Top-20 Feature Dims", color="#f0f0f0", fontsize=9, fontweight="bold"
    )
    ...
    ax2.fill_between(
        range(len(sorted_emb)), sorted_emb, alpha=0.6, color="#7c3aed", linewidth=0
    )
    ax2.plot(sorted_emb, color="#9b5de5", linewidth=1)
    ax2.set_title(
        "APOLLO Embedding Magnitude Distribution",
        color="#f0f0f0",
        fontsize=9,
        fontweight="bold",
    )
...
def plot_cellularity_survival(df: pd.DataFrame) -> io.BytesIO:
    ...
    ax.scatter(
        df["cellularity"], df["time"], c=colors, alpha=0.65, s=25, edgecolors="none"
    )
    ...
    ax.plot(
        x_line, m * x_line + b, color="#f4a261", lw=2, linestyle="--", label="Trend"
    )
    ...
    ax.set_xlabel("Cellularity", color="#aaaaaa")
    ax.set_ylabel("Survival Time (days)", color="#aaaaaa")
    ax.set_title(
        "Cellularity vs Survival Time", color="#f0f0f0", fontsize=10, fontweight="bold"
    )
    ax.tick_params(colors="#888888")
    ax.spines[:].set_color("#333333")
...
def run_scientific_workflow(cohort: str, tissue_slide_id: str):
    ...
    # ── Step 1: Perception ──────────────────────────────────────────────────
    with st.status(
        "🔭 Phase 1 — WSI Perception & Foundation Model Inference", expanded=False
    ) as s:
        wsi = WSIEngine()
    ...
    with st.status(
        "🧠 Phase 2 — Knowledge Fabric Consultation & Hypothesis Formation",
        expanded=False,
    ) as s:
        ...
        if "discovery_engine" not in st.session_state:
            ...
            st.session_state.discovery_engine = FastDiscoveryEngine(
                config, knowledge_fabric=fabric
            )
    ...
    with st.status(
        "🛡️ Phase 3 — CodeAct Generation & AST Security Audit", expanded=False
    ) as s:
        try:
            stat_code = discovery_engine._generate_test_code(
                hypothesis, mock_df, iteration=0
            )
        except Exception:
            stat_code = ""
...
"""
        critic = CriticAgent()
        is_safe, msg = critic.evaluate_code(
            stat_code, expected_columns=list(mock_df.columns)
        )
    ...
    wt_mask = ~mut_mask
    lr_res = logrank_test(
        mock_df.loc[mut_mask, "time"],
        mock_df.loc[wt_mask, "time"],
    ...
    run_label = (
        f"Hypothesis #{len(st.session_state.discovery_runs) + 1} — BRAF in {cohort}"
    )
    ...
    st.session_state.discovery_runs.append(
        {
            ...
            "stat_code": stat_code,
            "summary": f"BRAF mutation analysis pending session-wide FDR update in {cohort} (p={p_val:.4f})",
        }
    )
    _refresh_discovery_run_fdr(alpha=0.05)
    current_run = st.session_state.discovery_runs[-1]

    # Update knowledge fabric and logs
    fabric.add_relationship(
        "BRAF", cohort, f"PROGNOSTIC_p{p_val:.3f}_{titan_features['primary_pattern']}"
    )
    st.session_state.narrative.log_discovery(
        hypothesis,
        f"KM p={p_val:.4f} | FDR q={current_run.get('adjusted_p_value', 1.0):.4f} | Mutated median={mut_times.median():.0f}d | WT median={wt_times.median():.0f}d",
    )
...
def render_dashboard():
    ...
    with st.sidebar:
        ...
        st.divider()
        run_btn = st.button(
            "🚀 Run Generative Discovery Loop", type="primary", use_container_width=True
        )
    ...
    with m1:
        render_metric_card(
            "Graph Entities", str(len(fabric.graph.nodes)), "🧬", "#4e73df"
        )
    with m2:
        render_metric_card(
            "Relationships", str(len(fabric.graph.edges)), "🔗", "#1cc88a"
        )
    with m3:
        render_metric_card(
            "Hypotheses Tested",
            str(len(st.session_state.discovery_runs)),
            "💡",
            "#f6c90e",
        )
    with m4:
        render_metric_card(
            "Sandbox Executions", str(st.session_state.get("runs", 0)), "⚡", "#e63946"
        )
    ...
    with col_main:
        ...
        with c_conf:
            st.markdown("##### 🟢 FDR-Validated Prognostic Findings")
            if confirmed_runs:
        ...
        if runs:
            ...
            with v1:
                ...
                st.image(
                    plot_kaplan_meier(active_run["mock_df"]), use_container_width=True
                )
                q_value = active_run.get("adjusted_p_value")
                q_text = (
                    f", FDR q-value = `{q_value:.4f}`" if q_value is not None else ""
                )
                st.markdown(
                    f"**Conclusion**: raw p-value = `{active_run['p_value']:.4f}`{q_text} (median survival: mutated `{active_run['mut_median']:.0f}` days vs WT `{active_run['wt_median']:.0f}` days)"
                )

            with v2:
                st.markdown("###### Slide Cellularity vs Outcomes")
                st.image(
                    plot_cellularity_survival(active_run["mock_df"]),
                    use_container_width=True,
                )
                st.markdown(
                    f"**Morphology Link**: Mann-Whitney U test p-value = `{active_run['mwu_p']:.4f}`"
                )
            ...
            with st.expander("🔍 Selected Slide Perception Details (TITAN & APOLLO)"):
                ...
                with t1:
                    st.image(
                        plot_tissue_radar(active_run["titan_features"]),
                        use_container_width=True,
                    )
                with t2:
                    st.image(
                        plot_apollo_embedding(
                            active_run["apollo_emb"], active_run["slide_id"]
                        ),
                        use_container_width=True,
                    )
        else:
            st.info(
                "Run the discovery loop in the sidebar to generate interactive KM plots and tissue correlations."
            )
    ...
    with col_side:
        ...
        with st.expander("💡 Live Knowledge Fabric — Interactive Graph", expanded=True):
            ...
            if edges_list:
                st.dataframe(
                    pd.DataFrame(edges_list), use_container_width=True, hide_index=True
                )
        ...
        if history:
            ...
            with f_stat:
                status_filter = st.selectbox(
                    "Filter by Status", ["All", "Confirmed", "Rejected", "Blocked"]
                )
            ...
            for idx, log in enumerate(reversed(history)):
                ...
                if "Blocked" in log or "BLOCKED" in log:
                    ...
                    try:
                        q_part = log.split("FDR q=")[1].split(" |")[0]
                        q_val = float(q_part)
                        if q_val < 0.05:
                    ...
                    # Fallback for older log entries without stored q-values
                    try:
...
def _render_chat_panel(cohort: str, fabric: KnowledgeFabric) -> None:
    ...
    quick_prompts = [
        (
            "📊 Interpret BRAF result",
            "What does the BRAF mutation result mean for this cohort?",
        ),
        (
            "📈 Explain KM curve",
            "Can you explain the Kaplan-Meier curve and what it shows?",
        ),
        ("🔬 Suggest next steps", "What experiments or analyses should I run next?"),
        (
            "⚠️ Flag limitations",
            "What are the key limitations of this analysis I should be aware of?",
        ),
    ]
