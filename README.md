# SPARK: Autonomous Pathology Agent Framework
...
1. Formulates hypotheses based on available clinical data across various cancer types (e.g., TCGA-SKCM, TCGA-BRCA, TCGA-LUAD, TCGA-GBM).
...
- **Data Access (`tools/gdc_client.py`)**: Automated fetcher for open-access clinical metadata using the GDC API. Supports dynamic querying across multiple TCGA projects (Skin, Breast, Lung, Brain).
...
```
*You can specify different cancer types using `--cancer` (options: `skin`, `breast`, `lung`, `brain`). This will run the agent and generate a specific ledger file (e.g., `ledger_TCGA-BRCA.json`).*
